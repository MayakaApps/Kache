...
// It is a modified version of HashMap from Kotlin native-wasm stdlib as of v1.8.20
internal actual class ReverseLinkedHashMap<K, V> private constructor(
    ...
    override actual val keys: MutableSet<K>
        get() {
            val cur = keysView
            return if (cur == null) {
    ...
    override actual val values: MutableCollection<V>
        get() {
            val cur = valuesView
            return if (cur == null) {
    ...
    override actual val entries: MutableSet<MutableMap.MutableEntry<K, V>>
        get() {
            val cur = entriesView
            return if (cur == null) {
    ...
    internal open class Itr<K, V>(
        internal val map: ReverseLinkedHashMap<K, V>
    ) {
    ...
    internal class KeysItr<K, V>(map: ReverseLinkedHashMap<K, V>) : Itr<K, V>(map), MutableIterator<K> {
        override fun next(): K {
    ...
    internal class ValuesItr<K, V>(map: ReverseLinkedHashMap<K, V>) : Itr<K, V>(map), MutableIterator<V> {
        override fun next(): V {
            if (index < 0) throw NoSuchElementException()
            lastIndex = index--
    ...
    internal class EntriesItr<K, V>(map: ReverseLinkedHashMap<K, V>) : Itr<K, V>(map),
    ...
    internal class EntryRef<K, V>(
        private val map: ReverseLinkedHashMap<K, V>,
...
internal class HashMapKeys<E> internal constructor(
    private val backing: ReverseLinkedHashMap<E, *>
...
internal class HashMapValues<V> internal constructor(
    val backing: ReverseLinkedHashMap<*, V>
...
internal abstract class HashMapEntrySetBase<K, V, E : Map.Entry<K, V>> internal constructor(
    val backing: ReverseLinkedHashMap<K, V>
) : MutableSet<E>, AbstractMutableSet<E>() {
...
internal class HashMapEntrySet<K, V> internal constructor(
    backing: ReverseLinkedHashMap<K, V>
...
private fun <E> Array<E>.resetAt(index: Int) {
    (@Suppress("UNCHECKED_CAST") (this as Array<Any?>))[index] = null
}

private fun <E> Array<E>.resetRange(fromIndex: Int, toIndex: Int) {
    @Suppress("UNCHECKED_CAST")
    (this as Array<Any?>).fill(null, fromIndex, toIndex)
}

@Suppress("UNCHECKED_CAST")
private inline fun <E> arrayOfUninitializedElements(size: Int): Array<E> {
    require(size >= 0) { "capacity must be non-negative." }
    @Suppress("TYPE_PARAMETER_AS_REIFIED")
    return arrayOfNulls<E>(size) as Array<E>
}

private fun <T> Array<T>.copyOfUninitializedElements(newSize: Int): Array<T> {
    return copyOfUninitializedElements(0, newSize)
}

private fun <T> Array<T>.copyOfUninitializedElements(fromIndex: Int, toIndex: Int): Array<T> {
